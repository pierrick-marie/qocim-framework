...
public class RandomFreshnessFactory implements IQoCIMFactory {

	// # # # # # PRIVATE VARIABLES # # # # #

	public synchronized static RandomFreshnessFactory instance() {
		// - - - - - CORE OF THE METHOD - - - - -
		if (instance == null) {
			instance = new RandomFreshnessFactory();
		}
		// - - - - - RETURN STATEMENT - - - - -
		return instance;
	}
	...
	private static RandomFreshnessFactory instance = null;

	// # # # # # PUBLIC METHODS # # # # #

	private RandomFreshnessFactory() {
	}
	...
	public synchronized FreshnessQoCMetricValue newQoCMetricValue(final String _qoCMetricValueId,
		...
		try {
			String message = "FreshnessRandomFactory.newQoCMetricValue(Integer, Double): the argument _qoCMetricValueId is null.";
			ConstraintChecker.notNull(_qoCMetricValueId, message);
			message = "FreshnessRandomFactory.newQoCMetricValue(Integer, Double): the argument _qoCMetricValue is null.";
	...
	public FreshnessQoCIndicator newQoCIndicator(final String _qoCMetricValueId, final Double _qoCMetricValue) {
		// - - - - - CHECK THE VALUE OF THE ARGUMENTS - - - - -
		try {
			String message = "FreshnesRandomValueFactory.newQoCIndicator(Integer, Double): the argument _qoCMetricValueId is null";
			ConstraintChecker.notNull(_qoCMetricValueId, message);
			message = "FreshnesRandomValueFactory.newQoCIndicator(Integer, Double): the argument _qoCMetricValue is null";
			ConstraintChecker.notNull(_qoCMetricValue, message);
		} catch (final ConstraintCheckerException _exception) {
